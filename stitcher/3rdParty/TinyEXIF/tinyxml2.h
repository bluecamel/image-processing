...
private:
    ...
    void EnsureCapacity(int cap)
    {
        ...
        if (cap > _allocated) {
            ...
            memcpy(newMem, _mem, sizeof(T) * _size); // warning: not using constructors,
                                                     // only works for PODs
            if (_mem != _pool) { delete[] _mem; }
            _mem = newMem;
...
/**
        ...
        If you return 'true' from a Visit method, recursive parsing will continue. If you
   return false, <b>no children of this node or its siblings</b> will be visited.

        All flavors of Visit methods have a default implementation that returns 'true'
   (continue visiting). You need to only override methods that are interesting to you.
...
public:
    ...
    // Anything in the high order range of UTF-8 is assumed to not be whitespace. This
    // isn't correct, but simple, and usually works.
...
public:
    ...
    */
    ...
    /// Get the previous (left) sibling element of this node, with an optionally supplied
    /// name.
    ...
    /// Get the next (right) sibling element of this node, with an optionally supplied
    /// name.
    ...
    */
    ...
    /** Accept a hierarchical visit of the nodes in the TinyXML-2 DOM. Every node in the
       ...
       re-parse the XML for the callbacks, so the performance of TinyXML-2 is unchanged by
       using this interface versus any other.)
...
public:
    ...
    */
    ...
    /**
        ...
        The QueryIntText() and similar functions provide a safe and easier way to get to
       the "value" of x and y.
        ...
        @returns XML_SUCCESS (0) on success, XML_CAN_NOT_CONVERT_TEXT if the text cannot
       be converted to the requested type, and XML_NO_TEXT_NODE if there is no child text
       to query.
...
{
public:
    /// Create a handle from any node (at any depth of the tree.) This can be a null
    /// pointer.
...
/**
        A variant of the XMLHandle class for working with const XMLNodes and Documents. It
   is the same in all regards, except for the 'const' qualifiers. See XMLHandle for API.
*/
