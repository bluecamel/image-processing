...
Camera CameraModels::ParrotAnafiThermal(bool distortion_model_enabled)
{
    ...
    cv::Mat calibration_intrinsics =
        (cv::Mat_<double>(3, 3) << 2.9803291905661031e+03, 0.0,
         2.3574166587045511e+03, 0.0, 2.9704815213723482e+03,
         1.6709805782521339e+03, 0.0, 0.0, 1.0);
...
Camera CameraModels::VantageVesperEONavigation()
{
    ...
    std::vector<double> pol = {-2.863252e+03, 0.000000e+00, 4.060925e-04,
                               -2.040622e-07, 5.342978e-11};
    std::vector<double> inv_pol = {
        -2319.757678,   -49354.733874,  -202871.366882, -464932.147854,
        -669902.465355, -625830.239054, -379865.964545, -145131.700716,
        -31852.275059,  -3077.921163};
    ...
    auto distortion_model = std::make_shared<ScaramuzzaDistortionModel>(
        distortion_parameters, true,
        [height](const cv::Mat &image) -> cv::Rect {
            return cv::Rect(cv::Point(0, 0),
                            cv::Point(image.size().width,
    ...
    return Camera(focal_length_meters, sensor_dimensions_meters,
                  sensor_dimensions_pixels, principal_point, nullptr,
