...
CvMatCompare(const char *a_expr, const char *b_expr, const cv::Mat &a,
             const cv::Mat &b, const bool negate)
{
    if (a.rows != b.rows) {
        if (negate) {
        ...
        return ::testing::AssertionFailure()
               << a_expr << " and " << b_expr
               << " have a different number of rows (a: " << a.rows
               << ", b: " << b.rows << ")";
    }
    ...
    if (a.dims != b.dims) {
        ...
        return ::testing::AssertionFailure()
               << a_expr << " and " << b_expr
               << " have different dimensions (a: " << a.dims
               << ", b: " << b.dims << ").";
    }
    ...
    if (a.size != b.size) {
        ...
        return ::testing::AssertionFailure()
               << a_expr << " and " << b_expr
               << " have different sizes (a: " << a.size << ", b: " << b.size
               << ").";
    }
    ...
    if (a.elemSize() != b.elemSize()) {
        ...
        return ::testing::AssertionFailure()
               << a_expr << " and " << b_expr
               << " have different element sizes (a:" << a.elemSize()
               << ", b: " << b.elemSize() << ").";
    }
    ...
    if (a.isContinuous() && b.isContinuous()) {
        if (0 != memcmp(a.ptr(), b.ptr(), a.total() * a.elemSize())) {
            ...
            return ::testing::AssertionFailure()
                   << a_expr << " and " << b_expr << " differ";
        }

        if (negate) {
            return ::testing::AssertionFailure()
                   << a_expr << " and " << b_expr << " don't differ.";
        }
